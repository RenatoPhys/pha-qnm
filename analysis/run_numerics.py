...
def profile_on_grid(bg, nodes):
    ...
    horizon = ~positive
    values[:, horizon] = np.asarray(
        [0.0, 0.0, phi0, 0.0, hd["A1"], 1.0, hd["scalar1"], Phi1]
    )[:, None]
    return values
...
def main():
    ...
    summary = {
        ...
        "scope": "legacy decoupled baseline; claim-bearing Route-A coupled spectra are generated by validate_coupled_qnms.py and run_finite_k_physics.py",
    }
